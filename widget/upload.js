 ...
 * <div id="container">
        上传安装包：
        <a id="selectfiles" data-upType="2" href="javascript:void(0);" class='btn btn-flat btn-info'>上传文件</a>
    </div>
    <input type="hidden" id="apkurl" name="apkurl" title='文件上传成功后路径'/>
    <div id="file-list" class="cf" title='图片显示'></div>
 */
accessid = ''
accesskey = ''
host = ''
...
function send_request(upType)
{
    ...
    {//http://192.168.10.24:81/stat/uploadkey//http://ad.miduo.com/auth/getosskey
        serverUrl = '/auth/getosskey?type='+upType;
        xmlhttp.open( "GET", serverUrl, false );
        xmlhttp.send( null );//type=1是图片type=2是安装包
        var res = eval("("+xmlhttp.response+")");//跨域处理
        return res.data;
    }
    else
    {
            ...
            for (var i = 0, len = files.length; i < len; i++) {
                ...
                if(!/image\//.test(fileType)){
                    ...
                    if(files[i].size > 1073741824){
                        ...
                        ! function(i) {
                            $fileList.html($fileList.html() +'<div id="' + files[i].id + '">' + files[i].name + ' (' + plupload.formatSize(files[i].size).toUpperCase() + ')<b></b>'
