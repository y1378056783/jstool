...
}};Circles.create=function(options){return new Circles(options);};
  return function (num,per){
		var colors = ['#cccccc','#009bff'],percentage=per;
		for (var i = 1; i <= num; i++) {
		    var child = document.getElementById('circles-' + i);
		    Circles.create({
		        id:         child.id,
		        percentage: percentage[i-1],
		        radius:     27,//控制圆直径例如：55px/2 = 27
		        width:      6,
		        number:     percentage[i-1] / 1,
		        text:       '%',
		        colors:     colors
		    });  
		}
