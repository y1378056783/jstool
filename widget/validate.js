...
var validate = function(){
    ...
    this.emptyExp = /^\S+$/;//非空验证
    this.idExp = /^(\d{14}|\d{17})(\d|[xX])$/;//身份证
    this.bankExp =  /^([1-9]{1})(\d{14}|\d{18})$/;//银行卡
    this.phoneExp = /^0*(13|14|15|17|18)\d{9}$/;//手机号
    this.emailExp = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/;//邮箱
    this.msg = {
        "nameMsg":"真实姓名不正确",
        "emptyMsg":"支付宝账号不正确",
        "idMsg":"身份证号不正确",
